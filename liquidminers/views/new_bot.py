...
def new_bot(request):
    ...
    fiat_locked = Investment.objects.filter(user=user, pool=pool).aggregate(Sum('fiat_amount'))
    if fiat_locked['fiat_amount__sum'] is None:
        fiat_locked['fiat_amount__sum'] = 0
    context['fiat_locked'] = fiat_locked['fiat_amount__sum']
    coin_locked = Investment.objects.filter(user=user, pool=pool).aggregate(Sum('coin_amount'))
    if coin_locked['coin_amount__sum'] is None:
        coin_locked['coin_amount__sum'] = 0
    context['coin_locked'] = coin_locked['coin_amount__sum']
    context['exchange_name'] = pool.exchange.title()
    context['yield'] = Statistic.get_weekly_yield_of_pool(pool.id)
    context['price'] = round(price, 5)
    context['op_price'] = round(1.0 / price, 5)
    context['liquidity'] = Statistic.get_liquidity_of_pool(pool.id)
    context['day_volume'] = Statistic.get_day_volume_of_pool(pool.id)
    ...
    context['volume_history'] = Statistic.objects.filter(lookup_id=pool_id, name='archive_daily_pool_volume').order_by('datetime')

    context = {**context, **RewardSharing.get_reward_sharings(pool, user, 10)}
    if is_user_invested:
        context = {**context, **Order.get_all(pool, user)}

    return render(request, 'pages/new_bot.html', context)
